...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@forkencode/llm"
import type { LLMClientShape } from "@forkencode/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @forkencode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @forkencode/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
